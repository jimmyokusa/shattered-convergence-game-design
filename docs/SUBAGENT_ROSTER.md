...
* **Phase:** `ACTIVE-M0`
* **Mandate:** Maps character mechanics onto UFE 2 visual editors, documents config steps, authors deterministic C# code where native UFE features fall short, actively monitors `unity/Logs/Editor.log` to catch compilation errors, and executes the NUnit test suite on every code iteration.
...
* **Forbidden actions:**
  ...
  * MUST NOT leave generated C# editor scripts unverified against `unity/Logs/Editor.log` or without running automated tests.
...
* **Deliverable format:** UFE Editor Step-by-Step Configuration Guide or C# Code Snippet + Log & NUnit Test Audit.
* **Definition of done:** Move mapping is fully specified with explicit UFE Editor names, all C# editor scripts pass compilation cleanly in `unity/Logs/Editor.log`, and all NUnit tests in `MilestoneBuildTests.cs` pass 100%.
